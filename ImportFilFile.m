function hps = ImportFilFile(filFile, elecNum, selElecs, startT, winT)
%% ImportFilFile
% Loads data from an .fil file

%% Syntax
%# hps = ImportFilFile(filFile, elecNum, selElecs, startT, winT)

%% Description
% Extracts high pass traces from a .fil file at particular time points and
...
% obtained that will keep all returned high passed traces the same length.

%% INPUT
% * filFile - a string, the name of the .fil file
...
%% OUTPUT
% * hps - a structure with the following fields:
%     * traces - an Nx1 cell array, each cell contains the high pass traces across the
...
tFile = [filFile(1:(end-4)) '_t.dat'];
filFID = fopen(filFile, 'r');
...
fFilProps = dir(filFile);
fFilLength = fFilProps.bytes;
filLen = fFilLength/(2*elecNum); % number of eeg samples
fTProps = dir(tFile);
...
if filLen ~= tLen
    error('fil and t.dat files are unmatched, exiting');
end
...
for j = 1:numTraces
    ...
    datInd = startInd*elecNum*2;
    stepSize = winLen(j);
    fseek(filFID, datInd, -1);
    traces{j} = fread(filFID, [elecNum,stepSize], 'int16');
