function ops = ConfigureOps(fPath, datName, chanCount, chanMapName)
...
% ops.chanMap = 1:ops.Nchan; % treated as linear probe if unavailable chanMap file		
    ...
    if chanCount <= 8
        ...
        ops.Nfilt = chanCount*2; % number of clusters to use (2-4 times more than Nchan, should be a multiple of 32)
    end
    
ops.nNeighPC            = min([12 chanCount]); % visualization only (Phy): number of channnels to mask the PCs, leave empty to skip (12)		
ops.nNeigh              = min([16 chanCount]); % visualization only (Phy): number of neighboring templates to retain projections of (16)		
